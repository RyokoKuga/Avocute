...
def create_BL_Window():
    ...
    def batch_calc():
        ...
        for child in  bl_entry_table.get_children():
            ...
            subprocess.run(["mopac6.exe", bl_path])
            ...
            if(os.path.exists("FOR012")):
                ...
                # arcファイル移動
                bl_move_path = os.path.dirname(bl_path)
                bl_arc_path = os.path.join(bl_move_path, bl_arc_name)  
    ...
    def bl_mopac_run():
        ...
        if os.path.exists("mopac6.exe"):
            # 存在する場合はステータスチェックから実行
            bl_status_chk()
        
        # MOPACが見つからないときはエラー表示
        else:
            messagebox.showerror('Error', 'No mopac6.exe!!')
...
def create_ATS_Window():
    ...
    def frq_run():
        # MOPAC実行(frq)
        subprocess.run(["mopac6.exe", frq_filename])
    ...
    def ts_run():
        ...
        # MOPAC実行(TS)
        subprocess.run(["mopac6.exe", ts_filename])
    ...
    def saddle_run():
        ...
        # MOPAC実行(SADDLE)
        subprocess.run(["mopac6.exe", s_filename])
    ...
    def product_run():
        ...
        # MOPAC実行(生成物)
        subprocess.run(["mopac6.exe", p_path])
    ...
    def reactant_run():
        ...
        # MOPAC実行(反応物)
        subprocess.run(["mopac6.exe", r_path])
...
def about_Window():
    ...
    # ラベル
    appname_label = tk.Label(about_window, text = "AVOCUTE (Version : 1.0.0)").place(x = 25, y = 405)
    ...
    about_label = tk.Label(about_window, text = "AVOCUTE is a simple launcher for MOPAC.").place(x = 25, y = 450)
    mopac_label = tk.Label(about_window, text = "MOPAC (Version : 6.0.0)").place(x = 310, y = 405)
    mopacurl_label = tk.Label(about_window, text = "http://openmopac.net/", foreground = "#3366BB")
    mopacurl_label.place(x = 310, y = 425)
    mopacurl_label.bind("<Button-1>", lambda e: callback("http://openmopac.net/"))

    # 閉じるボタン
    close_button = tk.Button(about_window, text ="Close",  width = 8, command = about_close).place(x = 227, y = 480)
...
def mopac_run():
    ...
    if os.path.exists("mopac6.exe"):
        ...
        if status != "  No input file!!":
            ...
            # MOPAC実行
            subprocess.run(["mopac6.exe", dat_path])
            ...
            if(os.path.exists("FOR012")):
                ...
                # arcファイル移動
                move_path = os.path.dirname(dat_path)
                arc_path = os.path.join(move_path, arc_name)
...
def clear_yesno():
    # ダイアログで削除のyes/no
    ret = messagebox.askyesno("Clear Temporary Files", 'Are you sure you want to delete the FOR ＆ SHUTDOWN Files?')
    # yesなら削除
    if ret == True:
        file_list = glob.glob("FOR0*")
        for file in file_list:
...
# JSmol(web)へアクセス
def jsmol_web():
    jsmol_url = "https://chemapps.stolaf.edu/jmol/jmol.htm"
...
# File_Menuの内容
filemenu.add_command(label = "Open Input File... (*.dat)", command = load_Input)
...
# コンボボックス作成
calc_dict = {"Equilibrium Geometry": "EF", "Frequencies": "FORCE", "Transition State": "TS", "Saddle Point": "SADDLE"}
...
v4 = tk.StringVar()
cb4 = ttk.Checkbutton(options_frame, text = "VECTORS", variable = v4, onvalue = " VECTORS", offvalue = "", padding = 10)
cb4.grid(row=0, column=3)

v5 = tk.StringVar()
cb5 = ttk.Checkbutton(options_frame, text = "UHF", variable = v5, onvalue = " UHF", offvalue = "", padding = 10)
